...
| 5 | KRX 과거 일자 **소급 가능 범위** (조회 자체는 09-20 확인) | 호출 몇 회 |
...
| 9 | DART 목록 API 엔드포인트·파라미터 (`pblntf_detail_ty` 필터) | 필터 값만 있고 호출 방법이 없다 |
| 10 | 금투협 조회 창 1년을 **서버가 실제로 강제하는지** | 확인 1회. 백필 일정이 여기 달렸다 |
| 11 | 공공데이터포털 `beginBasDt` 증분 동작 | 한 번도 써 본 적이 없다 |
| 12 | KRX 응답 루트 키·필드 명세 | 명세 미확보. 지금은 방어 코드로 버틴다 |
| 13 | 금감원 `result` 단건 시 형태, 한 응답의 건수 상한 | 인증키 |
