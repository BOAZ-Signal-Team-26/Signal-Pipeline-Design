...
| KRX ETF 일별매매정보 | 키 + **서비스별 승인** | 실호출 확인 (1,167건) | **미확보** — 응답 구조는 방어 코드로 버틴다 | `verify_etf_rule.py` |
...
| 금감원 분쟁조정결정례 | 불필요 | 실호출 확인 (8건) | 없음 — **호출 경로조차 유실** (6절) | `hwp_text.py` + `_ole.py` |
...
| `vStrtDt` / `vEndDt` | `YYYYMMDD` | **조회기간 1년 이내** — 단 근거는 화면의 검증 규칙이고 **서버가 실제로 거절하는지는 미실측**이다(아래) |
...
**기한 리스크 — 지금 알아야 하는 것.** 조회 창이 1년이면, 받지 않고 1년이 지난 공시는
받을 수 없다. 백필 계획은 관문 B 사안이지만 늦어질수록 잃는 구간이 늘어난다(`gate-b/README.md`).
**다만 근거가 화면 JS의 검증 규칙뿐이다.** 서버가 1년을 넘는 요청을 실제로 거절하는지는
확인하지 않았다 → 10절 미확인 10. 창이 실제로는 더 넓을 수도 있으므로 **확인 1회가 백필
일정을 통째로 바꿀 수 있다.**
...
- **증분**: 워터마크는 `basDt`이고, 증분을 거는 **요청 변수는 `beginBasDt`**로 알려져 있다(`00b`).
  **스크립트는 전건 스캔이라 이 변수를 한 번도 써 본 적이 없다** → 10절 미확인 11.
...
| `{"respMsg":"Unauthorized **API Call**"}` | 키는 맞으나 **해당 서비스 승인이 없음** |
  ...
  **두 메시지를 구분해야 원인을 안다.**
- **응답**: 쓰는 필드는 `ISU_CD`(종목코드)·`ISU_NM`.
  **응답 루트 키와 전체 필드 명세를 확보하지 못했다.** 스크립트가 「최상위 객체 안의 유일한 배열」을
  목록으로 집는 것은 확인된 구조가 아니라 **방어 코드**다 → 10절 미확인 12.
...
- **과거 일자 조회는 된다 (09-20 확인).** `03`이 미확인으로 두고 있었으나, `13`의 재현 명령이
  **09-19에 `--base-date 20260904`로 실행해 1,167건을 재현**했다. 15일 전 일자가 돌아온 것이다.
  다만 **소급 가능 범위(며칠까지 거슬러 올라가는지)는 모른다.** 「전건 보관」 결정은 그대로 둔다 —
  범위를 모르는 채 보관을 느슨하게 하면 잃고 나서야 안다.
...
`14` 5절 실측은 임시 `curl`로 했고 스크립트로 남기지 않았다. 남은 단서는 게시판 ID(`B0000390`)와
그때 받은 `nttId` 8건뿐이다 — `229094` `229093` `219340` `218695` `217927` `217926` `217925` `136595`.
**재현은 이 값들에서 출발한다.** **수집 모듈을 짜기 전에 이 절을 채워야 한다** — 다른 소스와 달리
여기만 「다시 알아내야」 한다.
...
| 5 | KRX 과거 일자 **소급 가능 범위** (조회 자체는 09-20 확인) | 호출 몇 회 | `basDd`를 1년·3년 전으로 넣어 본다 |
| 6 | 국가법령정보 연결 방식 | **인증키가 아니라 설계 결정** (7절) | 미팅 안건. 키를 받아도 안 풀린다 |
| 7 | finlife `fin_prdt_cd` 공통 필드 여부 | 소스 존치 결정이 먼저 | 존치로 결정되면 그때 명세를 연다 |
| 8 | **분쟁조정 게시판 목록·첨부 URL과 파라미터** | 아무것도 막지 않는다. **기록하지 않아 잃어버린 것**이다 (6절) | `nttId` 8건에서 역추적 |
| 9 | DART 목록 API 엔드포인트·파라미터 (`pblntf_detail_ty` 필터) | 필터 값만 있고 호출 방법이 없다 | OPEN DART 가이드에서 목록 API를 찾는다 |
| 10 | 금투협 조회 창 1년을 **서버가 실제로 강제하는지** | 아무것도 막지 않는다 | 2년 범위로 1회 호출. **백필 일정이 여기 달렸다** |
| 11 | 공공데이터포털 `beginBasDt` 증분 동작 | 아무것도 막지 않는다 | 파라미터를 넣어 건수가 줄어드는지 본다 |
| 12 | KRX 응답 루트 키·필드 명세 | 명세 미확보 | 응답 1건을 그대로 저장해 키를 적는다 |
