...
| J10 | KRX ETF API / 공공데이터포털 시세 | ISU_CD | 상품.isu_cd | **코드** | 대기(주영) | ETF에만 존재. ISU_CD ↔ short_code를 잇는 소스가 없다. 둘 다 가진 표를 찾거나 ETF 종목명 문자열로 이어야 함 → 미확인 2 |
...
**집계**: 14행 중 확인됨 7행, 대기 7행. 대기 7행 중 J2·J7·J10·J12는 소스를 열면 바로 판정되고, J8·J13·J14는 판정 이전에 「소스가 무엇인가 / 조인이 맞는가」를 먼저 정해야 한다.
...
19. ~~금투협 15행이 클래스 개수인지 페이지 크기인지~~ → **해소(12)**: 클래스 개수다. 미래에셋밸런스롱숏 2026-08-14은 모펀드 1행 + 클래스 14행 = 15행으로 직접 확인. 페이지 크기 반론은 기각(API가 3일치 1,499행을 무페이징 반환). 단 15는 상수가 아니며 묶음 크기는 1~234로 흩어진다. **남은 것: 그 15행이 동일 PDF 파일을 가리키는지**는 첨부 다운로드 경로를 못 뚫어 미확인(12 6절). 02의 「같은 해시면 1회 저장」 규칙과 CAS 채택(11 충돌 4)이 여기 걸려 있다.
20. ~~금투협 tmpV1이 kofia_fund_code인지 standard_code인지~~ → **해소(12)**: 둘 다 아니라 **공시 유형에 따라 의미가 바뀐다**. 수시(`2OF`)에서는 모펀드 코드, 정기(`2RF`)에서는 자기 코드다. 코드 체계도 K55·KR5·KRM이 섞인다. **새 미확인**: `tmpV1`이 `ZZZ`로 시작하는 33행의 의미. 자연키에 tmpV1이 들어가므로 이 33행이 한 묶음으로 뭉칠 위험이 있어 적재 전 확인이 필요하다.
