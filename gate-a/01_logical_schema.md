...
| J12 | 금감원 검사결과제재 OPEN API | `finInstName`(금융회사) | 문서.distributor_id | **문자열** | 확인됨(14) | 금감원 공개 샘플로 판정. **표기는 법인격 없는 약칭(`교통은행`)이라 05 9절 1~4단계가 사실상 무동작**이고 5단계만 남는다. 응답 13개 필드에 **상품을 가리키는 칸이 없고, 본문 `actObjContent`도 `㉮펀드`로 마스킹**이다 → `문서.product_id`는 영구 NULL. 제재는 **판매사 단위 신호**다. 교차 확인 수단은 `actReqDate` 하나뿐 |
