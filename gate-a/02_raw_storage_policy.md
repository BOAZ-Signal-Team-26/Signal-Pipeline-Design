...
**관문 B 후보 (00b A4)**: 사람이 읽는 논리 경로를 `raw/{source}/{yyyy}/{mm}/{dd}/{source_doc_key}/{filename}` 형태로 세분화하고 파일 실체를 `blobs/{sha256 앞 2자}/{sha256}`에 두는 콘텐츠 주소 저장(CAS) 구조는 09-16에 채택하지 않는다. 금투협 15행이 실제로 동일 PDF인지가 아직 검증되지 않았고, 원본이 불변 보존되는 한 나중에 `blobs/`로 재배치하고 `storage_path`를 채우는 것은 되돌릴 수 있는 결정이기 때문이다. 채택 여부는 관문 B에서 다시 판단한다.
...
**결정 (아키텍트, 09-16 반영)**: 금투협 클래스 행 N개가 같은 PDF를 가리키는 경우, sha256으로 1회만 저장·추출한다.
**근거**: 금투협 수시공시는 클래스 행 단위로 여러 행이 조회되지만(관측상 15행) 실제 PDF는 1개일 가능성이 크다(00b A1). 해시가 같으면 같은 파일이므로 중복 다운로드·중복 추출을 피한다. 09-15 실측(00b A7, 금투협 15행 동일 PDF 여부)으로 확정한다.
