...
| **예약된 표 2개 (09-22)** | 14개 표에 자리 없음. score PK가 (실행, 절, 점수유형)으로 절 단위 고정 | ① 문서/문서쌍 관측값 표(문서 순서 준수·요약-본문 중복도) — 계산 단위 확정 뒤(다빈 09-23). ② 검증 결과 표(사람 라벨 3쌍 윌콕슨·LLM 12문항) — 검증 방법 확정 뒤. CDI 배점·임계값·가중치·방향·자산 버전은 score_payload·config_manifest JSON으로 흡수되어 표 변경 없음 |
