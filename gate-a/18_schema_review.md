...
| 고지충실도 | 09-23 사전 안건에서 점수화 방향 명시. 기존 score_type은 계약 확정 전 유지 | 민석·다빈: 배점·분모·계산 불가 상태·법적 서류 대응·적용 상품군. 점수화 방향과 산출 준비 완료를 구분(19) |
...
| **확장 영역 2개 (09-23 재검토)** | 문서/문서쌍 지표와 사람·LLM 검증. 표 개수 미정. score PK는 score_id, (run_id, section_id, score_type)은 UNIQUE | 계산 단위·최종 문서 점수·계산 불가 상태·검증 원응답 계약부터 확정. 배점 등은 같은 입도/출력 개수일 때 JSON으로 수용 가능. 기존 키·관계가 불변이라는 보장은 없음. [19 재검토](19_pending_decisions_review.md) |
